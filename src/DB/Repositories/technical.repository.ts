...
export class TechnicalRepository extends BaseRepository<TechnicalType> {
  constructor(
    @InjectModel(Technical.name)
    private readonly technicalModel: Model<TechnicalType>,
  ) {
  ...
  async findOneDocument(
    filters: Filter<TechnicalType>,
    project?: ProjectionType<TechnicalType>,
  ...
  async checkTechnicalExists(email: string, phone: string): Promise<boolean> {
  ...
  async updateProfile(
    id: Types.ObjectId,
    updateData: Partial<TechnicalType>,
  ): Promise<void> {
    await this.technicalModel.findByIdAndUpdate(id, updateData);
  }

  async updateManyTechnicals(
    filters: UpdateManyFilter<TechnicalType>,
    updateQuery: UpdateQuery<TechnicalType>,
  ): Promise<void> {
  ...
  async updateTechnicalStatus(
    ...
    await this.technicalModel.findByIdAndUpdate(
      id,
      { isActive },
      { new: true },
    );
